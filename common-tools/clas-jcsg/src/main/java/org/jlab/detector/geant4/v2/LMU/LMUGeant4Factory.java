...
//     for (int region = 1; region <= LMUConstants.NREGIONS; region++) {
//         // Select the appropriate thickness array based on region
//         double Thickness = (region == 1 || region == 6) ? 
//                                   LMUConstants.GEM_CHAMBERVOLUMESTHICKNESS : 
//                                   LMUConstants.URWELL_CHAMBERVOLUMESTHICKNESS;
        
//         // Sum up the thickness for this region
//         //for (double thickness : thicknessArray) {
//             chamberT += thickness;
//             System.out.println("Region " + region + "Total Thickness: " + chamberT + " cm");
...
//     }
    ...
    public double getChamberThickness() {
        ...
        // Loop through regions 1 to 6
        for (int region = 1; region <= 6; region++) {
            // Check for GEM or URWELL based on the region
            if (region == 1 || region == 6) {
                // Use GEM_CHAMBERVOLUMESTHICKNESS
                for (int i = 0; i < LMUConstants.GEM_CHAMBERVOLUMESTHICKNESS.length; i++) {
                    chamberT += LMUConstants.GEM_CHAMBERVOLUMESTHICKNESS[i];
                    System.out.println("Region " + region + " - Adding GEM thickness: " + LMUConstants.GEM_CHAMBERVOLUMESTHICKNESS[i] + " cm");
                }
            } 
            else {
                // Use URWELL_CHAMBERVOLUMESTHICKNESS for regions 2, 3, 4, 5
                for (int i = 0; i < LMUConstants.URWELL_CHAMBERVOLUMESTHICKNESS.length; i++) {
                    chamberT += LMUConstants.URWELL_CHAMBERVOLUMESTHICKNESS[i];
                    System.out.println("Region " + region + " - Adding URWELL thickness: " + LMUConstants.URWELL_CHAMBERVOLUMESTHICKNESS[i] + " cm");
                }
            }
            System.out.println("Region " + region + "Total Thickness: " + chamberT + " cm");
        }
    ...
    public double[] getRegionDimensions(int region) {
        double[] regionDimension = new double[3];    
    ...
    */
    ...
    public Geant4Basic createRegion(int iregion) {
        int regionIndex = iregion + 1;
        double[] dim = this.getRegionDimensions(regionIndex);
        ...
        Geant4Basic regionVolume = new G4Box("LMU_region" + regionIndex, regionDX, regionDY, regionDZ);
        regionVolume.translate(vCenter.x, vCenter.y, vCenter.z);
        regionVolume.setId(regionIndex, 0, 0);

        Geant4Basic chamberVolume = this.createChamber(iregion);
        chamberVolume.setName("region" + regionIndex);
        chamberVolume.setMother(regionVolume);
        chamberVolume.setId(regionIndex, 1, 0);
    ...
    public Geant4Basic createChamber(int iRegion) {
        int regionIndex = iRegion + 1;
        double chamberDX = LMUConstants.XSIZE / 2 + LMUConstants.XENLARGEMENT;
        double chamberDY = LMUConstants.YSIZE / 2 + LMUConstants.YENLARGEMENT;
        double chamberDZ = this.getChamberThickness() / 2.0;

        Geant4Basic chamberVolume = new G4Box("region" + regionIndex, chamberDX, chamberDY, chamberDZ);

        double[] thicknessArray = (regionIndex == 1 || regionIndex == 6) ? 
                                   LMUConstants.GEM_CHAMBERVOLUMESTHICKNESS : 
                                   LMUConstants.URWELL_CHAMBERVOLUMESTHICKNESS;

        String[] nameArray = (regionIndex == 1 || regionIndex == 6) ? 
                              LMUConstants.GEM_CHAMBERVOLUMESNAME : 
                              LMUConstants.URWELL_CHAMBERVOLUMESNAME;

        double daughterVolumeZ = -chamberDZ;
        for (int i = 0; i < thicknessArray.length; i++) {
            double daughterThickness = thicknessArray[i] / 2.0;
            daughterVolumeZ += daughterThickness;

            Geant4Basic daughterVolume = new G4Box("daughter_volume", chamberDX, chamberDY, daughterThickness);
            daughterVolume.setName("region" + regionIndex + "_" + nameArray[i]);
            daughterVolume.setMother(chamberVolume);
            daughterVolume.setPosition(0.0, 0.0, daughterVolumeZ);
